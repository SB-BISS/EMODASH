...
class Structures:
    ...
    def structure_11_cnn_attention_dot_old(self):
        # keras.layers.merge.Dot(axes, normalize=False)
        L = self.CONTEXT_LENGTH
        NEURONS = self.NEURONS
        encoding_input = Input(shape=(L, self.embedding_size))
    ...
    def structure_13(self):
        ...
        context_model.add(Convolution2D(32, 2, 2, activation='relu', border_mode='same',
                                        input_shape=(1, self.CONTEXT_LENGTH - 1, self.CONTEXT_LENGTH - 1)))
    ...
    def structure_15(self, batch_size):
        ...
        lstm_fwd = LSTM(128, batch_input_shape=(batch_size, self.CONTEXT_LENGTH - 1, self.embedding_size),
                        return_sequences=True, name='lstm_fwd', stateful=True)(drop_out)
        lstm_bwd = LSTM(128, batch_input_shape=(batch_size, self.CONTEXT_LENGTH - 1, self.embedding_size),
                        return_sequences=True, go_backwards=True, name='lstm_bwd', stateful=True)(drop_out)
        bilstm = merge([lstm_fwd, lstm_bwd], name='bilstm', mode='concat')
